...
for file in *.mkv;  do 
    ffmpeg -init_hw_device vaapi=drive:/dev/dri/renderD128 -hwaccel vaapi -hwaccel_output_format vaapi -hwaccel_device drive -i "$file" -filter_hw_device drive -vf 'format=nv12|vaapi,hwupload' -c:v h264_vaapi -c:s mov_text output.mp4;
    if [[ $? != 0 ]]; then exit 1 ; fi
    rm -f "$file"
