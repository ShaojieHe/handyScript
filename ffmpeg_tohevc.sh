...
for file in *."${name}"; do
    ffmpeg -init_hw_device vaapi=device:/dev/dri/renderD129  -hwaccel vaapi -hwaccel_device device -hwaccel_output_format vaapi -i "${file}" -filter_hw_device device -vf 'format=p010|vaapi,hwupload' -c:v hevc_vaapi -bf 0 -c:a copy -c:s -qp 52 mov_text "output.mp4"
    if [[ $? != 0 ]]; then exit 1 ; fi
    rm -f "${file}"
    if [[ $? != 0 ]]; then exit 1 ; fi
